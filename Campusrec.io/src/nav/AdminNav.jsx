...
export default function AdminNav() {
  ...
  const navLinkClass = ({ isActive }) =>
    ['chip-nav', isActive ? 'chip-nav-active' : 'chip-nav-idle'].join(' ');

  return (
    <header className="workspace-header">
      <div className="workspace-header-inner">
        <Link to="/admin" className="workspace-brand">
          <span className="workspace-mark">A</span>
          <span className="leading-tight">
            <span className="workspace-title">Campus Workspace</span>
            <span className="workspace-subtitle">Admin</span>
          </span>
        </Link>
        <nav className="hidden md:flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onLogout}
            className="btn-dark inline-flex items-center gap-1.5"
          >
      ...
      {open && (
        <div className="mobile-nav-panel text-gray-900 md:hidden">
          <nav className="page-wrap flex flex-col gap-2 py-4">
            ...
            <NavLink to="/admin/profile" className={navLinkClass} onClick={() => setOpen(false)}>
              <FiUser className="h-4 w-4" />
              Profile
            </NavLink>
            <ThemeToggle />
